...
// paginated 10 at a time. Called on page load, after every add/delete,
...
totalAmountEl.textContent = Number(data.totalAmount || 0).toFixed(2);
...
// Shows Prev/Next controls plus "Page X of Y (Z total)" so the user can
// always see the last page / total expense count.
...
expenseForm.reset();
// New expenses sort first (newest first), so jump back to page 1 to see it.
loadExpenses(1);
} catch (err) {
errorMsg.textContent = err.message;
}
...
// If that was the last item on a page beyond page 1, step back a page
// so the user doesn't land on a now-empty page.
const isLastRowOnPage = expenseTableBody.children.length === 1;
loadExpenses(isLastRowOnPage && currentPage > 1 ? currentPage - 1 : currentPage);
} catch (err) {
errorMsg.textContent = err.message;
}
});
